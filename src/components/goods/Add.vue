<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 步骤条区域，active为激活项的索引，强制转化为number类型，name属性与active属性共用activeIndex双向绑定实现步骤条跟进度条联动性 -->
      ...
      <!-- tab栏区域，value/v-model绑定值，选中选项卡的 name 类型string ，label-position控制文字位置,beforeTabLeave设置切换门槛，tab-click点击标签实现跳转 -->
        ...
        <el-tabs :tab-position="'left'" v-model="activeIndex" :before-leave="beforeTabLeave" @tab-click="tabClicked">
          ...
          <el-tab-pane label="基本信息" name="0">
            ...
            <el-form-item label="商品分类" prop="goods_cat">
              <!-- 选择商品分类的级联选择器，:options数据源，:props配置项 -->
          ...
          <el-tab-pane label="商品图片" name="3">
            <!-- action 表示图片要上传的后台API地址,on-preview预览时间，on-remove图片移除，on-success成功上传 -->
    ...
    <!-- 图片预览对话框-->
    <el-dialog title="图片预览" :visible.sync="previewVisible"><img :src="previewPath" alt="" class="previewImg"></el-dialog>
  </div>
</template>
...
export default {
  data () {
    return {
      ...
      addFormRules: {
        ...
        good_weight: [ { required: true, message: '请输入商品重量', trigger: 'blur' } ],
  ...
  methods: {
    ...
    // 切换标签之前的钩子，若商品分类不为3时提醒用户选择商品分类并返回 false阻止标签页的切换
    ...
    //获取商品的动态参数和静态属性，activeIndex为1时获取动态参数，activeIndex为2时获取静态参数
    async tabClicked () {
      // 判断访问的是商品参数面板
      if (this.activeIndex === '1'){ const { data: res } = await this.$http.get(`categories/${this.cateId}/attributes`, {params: { sel: 'many' } })
      ...
      } else if (this.activeIndex === '2') {const { data: res } = await this.$http.get(`categories/${this.cateId}/attributes`, { params: { sel: 'only' } })
    ...
    // 点击添加商品按钮将商品信息上传到数据库中
    add () {
      this.$refs.addFormRef.validate(async vaild => {
        if (!vaild) { return this.$message.error('请填写必要的表单项！') }
        // 执行添加的业务逻辑，处理goods_cats属性 将数组转化为以 , 分隔的字符串，addForm需要表单双向绑定只认字符串类型，用lodash 调用cloneDeep方法进行深拷贝
